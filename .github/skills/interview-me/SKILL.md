...
Adapted from [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills/tree/be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39/skills/interview-me)
at commit `be4e44a9fbc5e8df0beaefadbb28bd22ee61cc39`. See [LICENSE](LICENSE).
Local adaptations support cloud follow-up turns and policy-approved output.
...
- Ask one question in the response and wait for a follow-up turn. No special
  interactive tool or Copilot CLI command is required. If the task cannot
  receive a reply, report the missing input and stop rather than guessing.
- Return the intent statement in the conversation. Save a file only when the
  user explicitly requests its name or path and repository policy permits it.
  Do not capture confidential material in memory or proposed documents.
- Confidence numbers below describe a subjective working hypothesis, not a
  measured probability or permission to assert canonical memory confidence.
- Only `idea-refine` and `spec-driven-development` are installed companion
  skills here. Other upstream skill names below are optional context, not
  dependencies; do not claim to invoke an unavailable skill.
...
This skill needs user replies, which may arrive as cloud follow-up turns.
In unattended CI or scheduled runs, flag missing requirements as a blocker
instead of starting an interview that cannot receive answers.
...
If the user wants the intent to persist (a multi-session project, a handoff to another collaborator), offer to save it to a policy-permitted path such as `projects/[topic]-intent.md`. Only save if they explicitly request that file.
